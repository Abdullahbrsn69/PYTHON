...
""" # interest calculation, faiz hesaplama anlamına gelir. Bu fonksiyon ile kullanıcıların hesaplarında bulunan bakiyelerin faizini hesaplayabilirsiniz.
 Burada faiz kısa ve uzun vadeli olacaktır. 
 Kısa vadeli faiz hesaplaması için kullanıcıların hesaplarında bulunan bakiyeleri 100 TL'den büyükse,
 uzun vadeli faiz hesaplaması için kullanıcıların hesaplarında bulunan bakiyeleri 1000 TL'den büyükse hesaplanır. 
 Bu fonksiyonda kullanıcıların hesaplarındaki paraları bekletme süresine göre faiz hesaplaması yapılır."""

def interest_calculation(deposit_time):
    
    deposit_time = int(deposit_time)
    users = get_data()
    for account_number in users:
        if users[account_number]["balance"] >= 100:
            if deposit_time <= 12:
                users[account_number]["balance"] += users[account_number]["balance"] * 0.05
            elif deposit_time > 12:
                users[account_number]["balance"] += users[account_number]["balance"] * 0.1
        if users[account_number]["balance"] >= 1000:
            if deposit_time <= 12:
                users[account_number]["balance"] += users[account_number]["balance"] * 0.1
            elif deposit_time > 12:
                users[account_number]["balance"] += users[account_number]["balance"] * 0.2
    set_data(users)
    # şimdi kullanıcının bakiyesini ekrana yazdıralım.
...
def display_menu():
    ...
    if user_choice == 7:
        ...
        deposit_time = int(input("Deposit Time (in months): "))
        interest_calculation(deposit_time)
